...
namespace TiCodeX.SQLSchemaCompare.Test
{
    ...
    public abstract class DatabaseFixture : IDisposable
    {
        ...
        internal void CompareDatabases(DatabaseType type, string sourceDatabaseName, string targetDatabaseName, short port)
        {
            ...
            var userType = typeof(ABaseDbUser);
            switch (type)
            {
                case DatabaseType.MicrosoftSql:
                    ...
                    userType = typeof(MicrosoftSqlUser);
                    break;
                ...
                case DatabaseType.PostgreSql:
                    ...
                    userType = typeof(PostgreSqlUser);
                    break;
            }
            ...
            var users = sourceDb.Users.OrderBy(x => x.Schema).ThenBy(x => x.Name).Select(x => Convert.ChangeType(x, userType, CultureInfo.InvariantCulture));
            var clonedUsers = targetDb.Users.OrderBy(x => x.Schema).ThenBy(x => x.Name).Select(x => Convert.ChangeType(x, userType, CultureInfo.InvariantCulture));
            users.Should().BeEquivalentTo(clonedUsers, options =>
            {
                options.Excluding(x => ((ABaseDbObject)x).Database);
